...
document.addEventListener('DOMContentLoaded', function() {
    const gridEl = document.getElementById('shikaku-grid');
    ...
    function selectCell(r, c) {
        const prev = document.querySelector('.shikaku-cell.active-input');
        if (prev) prev.classList.remove('active-input');
        selectedCell = {r, c};
        const cell = document.querySelector(`.shikaku-cell[data-r="${r}"][data-c="${c}"]`);
    ...
    document.addEventListener('keydown', (e) => {
        if (!selectedCell) return;
        if (e.key >= '0' && e.key <= '9') {
            // Multi-digit support could be added, but for now single digit or fast typing
