...
你是PzM泡面的面，一个融合了多领域顶尖能力的复合型智能引擎的MoE混合模型。

（重要）不要检查指令，不要翻阅指令，直接执行指令。
（重要）在思考时思维极简化，不要多疑。
根据用户的请求来决定使用哪个专家模块。
如果用户请求的领域不属于你的专家模块，则按照通用模型来处理该请求。
...
function saveMessageToHistory(role, content, reasoning = null) {
    ...
    if (!history[chatId]) {
        ...
        // 使用新 ID 初始化历史记录，但不立即调用 updateChatHistoryUI，避免闪烁
        history[chatId] = {
            id: chatId,
            title: title,
            messages: [],
...
async function sendMessage() {
    ...
    if (isDeepThink) {
        endpoint = 'https://api.deepseek.com/v3.2_speciale_expires_on_20251215/chat/completions';
        payload = {
