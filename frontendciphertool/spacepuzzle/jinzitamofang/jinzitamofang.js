(function () {
    const PYRAMINX_STYLE_ID = 'pyraminx-module-style';
    const PYRAMINX_STYLES = `
/* 空间类：金字塔魔方 */
.space-pyraminx-panel {
  min-width: 420px;
  background:
...
.pyraminx-solid {
  ...
  height: 310px;
  position: relative;
  transform: scale(var(--pyraminx-scale, 1));
...
.pyraminx-projection-sticker {
  fill: var(--sticker-color);
  stroke: rgba(6, 16, 22, 0.82);
  stroke-width: 2.6;
  stroke-linejoin: round;
  filter:
    drop-shadow(0 0 6px rgba(64, 224, 255, 0.14))
    drop-shadow(0 0 3px rgba(255, 255, 255, 0.18));
}

.pyraminx-projection-label {
  ...
  letter-spacing: 1px;
  pointer-events: none;
  text-shadow: 0 0 8px var(--face-tint);
  text-anchor: middle;
...
.pyraminx-move-title small {
  color: rgba(255, 255, 255, 0.62);
  font-size: 0.72rem;
  font-weight: 600;
}

.pyraminx-move-actions {
  display: grid;
...
.pyraminx-move-chip:hover {
  transform: translateY(-2px);
  border-color: var(--face-color);
  background: rgba(64, 224, 255, 0.12);
  box-shadow: 0 0 12px rgba(64, 224, 255, 0.22);
}

#pyraminx-step-counter {
  color: #ffda59;
  font-weight: 800;
...
.pyraminx-step-panel .cyber-button:disabled {
  opacity: 0.45;
  cursor: not-allowed;
  filter: grayscale(0.45);
}

.pyraminx-step-panel .cyber-button:disabled:hover {
  transform: none;
}
...
`;
    ...
    function injectPyraminxStyles() {
        if (window.SpacePuzzleUI && typeof window.SpacePuzzleUI.injectStyles === 'function') {
            window.SpacePuzzleUI.injectStyles(PYRAMINX_STYLE_ID, PYRAMINX_STYLES);
            return;
        }
        if (typeof document === 'undefined' || document.getElementById(PYRAMINX_STYLE_ID)) return;
        const style = document.createElement('style');
        style.id = PYRAMINX_STYLE_ID;
        style.textContent = PYRAMINX_STYLES;
        document.head.appendChild(style);
    }

    injectPyraminxStyles();
    ...
    const TETRA_EDGE = 220;
    const TETRA_BASE_RADIUS = TETRA_EDGE / Math.sqrt(3);
    const TETRA_HEIGHT = TETRA_EDGE * Math.sqrt(2 / 3);
    const TETRA_VERTICES = {
        top: { x: 0, y: -TETRA_HEIGHT * 0.72, z: 0 },
        front: { x: 0, y: TETRA_HEIGHT * 0.28, z: TETRA_BASE_RADIUS },
        left: { x: -TETRA_EDGE / 2, y: TETRA_HEIGHT * 0.28, z: -TETRA_BASE_RADIUS / 2 },
        right: { x: TETRA_EDGE / 2, y: TETRA_HEIGHT * 0.28, z: -TETRA_BASE_RADIUS / 2 }
    };
    const FACE_VERTEX_KEYS = {
        U: ['top', 'left', 'right'],
        L: ['top', 'front', 'left'],
        R: ['top', 'right', 'front'],
        B: ['front', 'right', 'left']
    };
    const PYRAMINX_VIEWBOX = { width: 360, height: 310, perspective: 660 };
    ...
    const ROTATE_CW_MAP = [8, 3, 7, 6, 0, 2, 1, 5, 4];
    const ROTATE_CCW_MAP = [4, 6, 5, 1, 8, 7, 3, 2, 0];
    ...
    function divide3(v, scalar) {
        return { x: v.x / scalar, y: v.y / scalar, z: v.z / scalar };
    }

    function faceBasis(face) {
        const [apexKey, leftKey, rightKey] = FACE_VERTEX_KEYS[face];
        const apex = TETRA_VERTICES[apexKey];
        const left = TETRA_VERTICES[leftKey];
        const right = TETRA_VERTICES[rightKey];
        const axisX = divide3(subtract3(right, left), FACE_WIDTH);
        const origin = subtract3(apex, {
            x: axisX.x * FACE_WIDTH / 2,
            y: axisX.y * FACE_WIDTH / 2,
            z: axisX.z * FACE_WIDTH / 2
        });
        const axisY = divide3(subtract3(left, origin), FACE_HEIGHT);
        return { origin, axisX, axisY };
    }

    function localFacePointTo3D(face, point) {
        const { origin, axisX, axisY } = faceBasis(face);
        const x = point[0] / 180 * FACE_WIDTH;
        const y = point[1] / TRIANGLE_HEIGHT * FACE_HEIGHT;
        return {
            x: origin.x + axisX.x * x + axisY.x * y,
            y: origin.y + axisX.y * x + axisY.y * y,
            z: origin.z + axisX.z * x + axisY.z * y
        };
    }

    function rotate3D(point) {
        const rx = pyraminxRotX * Math.PI / 180;
        const ry = pyraminxRotY * Math.PI / 180;
    ...
    function project3D(point) {
        ...
        const scale = PYRAMINX_VIEWBOX.perspective / (PYRAMINX_VIEWBOX.perspective - rotated.z);
        return {
            x: PYRAMINX_VIEWBOX.width / 2 + rotated.x * scale,
            y: PYRAMINX_VIEWBOX.height / 2 + rotated.y * scale,
            z: rotated.z
        };
    }

    function projectedPolygonAttr(points) {
        return points.map(point => `${point.x.toFixed(2)},${point.y.toFixed(2)}`).join(' ');
    }

    function renderPyraminxView() {
        const solid = document.getElementById('pyraminx-solid');
        if (!solid) return;

        const polygons = [];
        FACE_NAMES.forEach(face => {
            TRIANGLE_POINTS.forEach((points, index) => {
                const projected = points.map(point => project3D(localFacePointTo3D(face, point)));
                polygons.push({
                    face,
                    index,
        ...
        const labels = FACE_NAMES.map(face => {
            const center = project3D(localFacePointTo3D(face, [90, TRIANGLE_HEIGHT * 0.66]));
            return { face, center };
        }).sort((a, b) => a.center.z - b.center.z);

        solid.innerHTML = `
            <svg class="pyraminx-projection" viewBox="0 0 ${PYRAMINX_VIEWBOX.width} ${PYRAMINX_VIEWBOX.height}" aria-label="金字塔魔方">
                ${polygons.map(item => `
                    <polygon class="pyraminx-projection-sticker"
                        points="${projectedPolygonAttr(item.points)}"
                        style="--sticker-color:${item.color};--face-tint:${FACE_COLORS[item.face]}"></polygon>
                `).join('')}
                ${labels.map(item => `
                    <text class="pyraminx-projection-label"
                        x="${item.center.x.toFixed(2)}"
                        y="${item.center.y.toFixed(2)}"
                        style="--face-tint:${FACE_COLORS[item.face]}">${item.face}</text>
                `).join('')}
            </svg>
        `;
    }
    ...
    function bindPyraminxDrag() {
        ...
        scene.addEventListener('pointermove', event => {
            ...
            pyraminxRotX = Math.max(-170, Math.min(170, startRotX - dy * 0.45));
    ...
    function getWorkspaceHTML() {
        return `
            <div id="jinzitamofang-workspace" class="space-workspace">
                ...
                <div class="grid-container card space-display-panel space-pyraminx-panel">
