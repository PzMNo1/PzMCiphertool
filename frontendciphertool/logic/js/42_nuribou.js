...
document.addEventListener('DOMContentLoaded', function() {
    const gridEl = document.getElementById('nuribou-grid');
    ...
    function selectCell(r, c) {
        const prev = document.querySelector('.nuribou-cell.active-input');
        if (prev) prev.classList.remove('active-input');
        selectedCell = {r, c};
        const cell = document.querySelector(`.nuribou-cell[data-r="${r}"][data-c="${c}"]`);
        if (cell) cell.classList.add('active-input');
    }

    function toggleShade(r, c) {
        const d = gridData[r][c];
        // Cycle: Unknown -> Shaded -> White -> Unknown
    ...
    function setNumber(num) {
        ...
        // If number set, it's definitely white (island)
        if (num !== null) gridData[r][c].state = 'white';
        renderCell(r, c);
    }
    ...
    solveBtn.addEventListener('click', () => {
        ...
        setTimeout(() => {
            ...
            msg.textContent = 'Solution would appear here.';
        }, 500);
    });
});
